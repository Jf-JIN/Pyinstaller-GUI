...
from PyQt5.QtWidgets import QDialog, QLabel, QPushButton, QVBoxLayout, QHBoxLayout, QWidget,  QSizePolicy, QMessageBox
...
from system.Manager_Style import STYLE


class DialogMessageBox(QDialog):
    class StandardButton(StaticEnum):
        RESET = 'reset'
        CANCEL = 'cancel'

    @staticmethod
    def question(parent, title: str = 'Qestion', content: str = '', buttons: list = [], default_button: int = -1, isResetable=False):
        ...
        dialog = DialogMessageBox(parent, title, content, buttons, icon, default_button, isResetable)
        dialog.exec_()
    ...
    def info(parent, content: str):
        ...
        dialog = DialogMessageBox(parent, LanguageManager.getWord('info'), content, buttons=[LanguageManager.getWord('certain')], icon=icon, default_button=-1, isResetable=False)
        dialog.exec_()
        if not hasattr(dialog, 'selected_btn'):
            return None
        return dialog.selected_btn
    ...
    def warning(parent, content: str):
        ...
        dialog = DialogMessageBox(parent, LanguageManager.getWord('warning'), content, buttons=[LanguageManager.getWord('certain')], icon=icon, default_button=-1, isResetable=False)
        dialog.exec_()
        if not hasattr(dialog, 'selected_btn'):
            return None
        return dialog.selected_btn
    ...
    def error(parent, content: str):
        ...
        dialog = DialogMessageBox(parent, LanguageManager.getWord('certain'), content, buttons=[LanguageManager.getWord('certain')], icon=icon, default_button=-1, isResetable=False)
        dialog.exec_()
        if not hasattr(dialog, 'selected_btn'):
            return None
        return dialog.selected_btn
    ...
    def critical(parent, content: str):
        ...
        dialog = DialogMessageBox(parent, LanguageManager.getWord('critical'), content, buttons=[LanguageManager.getWord('certain')], icon=icon, default_button=-1, isResetable=False)
        dialog.exec_()
        if not hasattr(dialog, 'selected_btn'):
            return None
        return dialog.selected_btn
    ...
    @staticmethod
    def show(parent, title: str, content: str, buttons: list, icon: str = None, default_button: int = -1, isResetable=False):
        dialog = DialogMessageBox(parent, title, content, buttons, icon, default_button, isResetable)
        dialog.exec_()
        if not hasattr(dialog, 'selected_btn'):
            return None
    ...
    def __init__(self, parent, title: str, content: str, buttons: list, icon: str = None, default_button: int = -1, isResetable: bool = False):
        ...
        pb_reset = QPushButton(LanguageManager.getWord('reset'))
        pb_reset.setProperty('widgetType', 'message_box')
        pb_reset.setCursor(Qt.PointingHandCursor)
        pb_reset.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        pb_reset.clicked.connect(lambda: self.__on_button_clicked(self.StandardButton.RESET))
        pb_cancel = QPushButton(LanguageManager.getWord('cancel'))
        pb_cancel.setProperty('widgetType', 'message_box')
        pb_cancel.setDefault(True)
        pb_cancel.setStyleSheet(STYLE.getBlock().get_item('@message_box_default_button').style)
        pb_cancel.setCursor(Qt.PointingHandCursor)
        pb_cancel.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        pb_cancel.clicked.connect(lambda: self.__on_button_clicked(self.StandardButton.CANCEL))
        ...
        for index, label in enumerate(buttons):
            ...
            button.setProperty('widgetType', 'message_box')
            ...
            if index == default_button:
                ...
                button.setStyleSheet(STYLE.getBlock().get_item('@message_box_default_button').style)
