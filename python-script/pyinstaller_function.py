...
class Pyinstaller_function(PyToExeUI):
    ...
    def version_display(self):
        ...
        display_threading = threading.Thread(target = self.read_output, args=(process, self.json_widgets['pb_VersionDisplay']['text_browser_display'],self.json_widgets['pb_VersionDisplay']['text_browser_cmd_display'],))
    ...
    def help_display(self):
        ...
        display_threading = threading.Thread(target = self.read_output, args=(process, self.json_widgets['pb_HelpDisplay']['text_browser_display'],self.json_widgets['pb_HelpDisplay']['text_browser_cmd_display'],))
        display_threading.start()
    ...
    def specpath(self):
        try:
            if self.cmd_dict['specpath'][0]:
                ...
                action_reply = self.set_custom_message_box(self.json_general['data_exist'], f'{self.json_general["current_data"]}{current_data}<br>{self.json_general["msg_file_replace"]}<br>', [self.json_general['pb_replace']], True)
                if action_reply == 0:
            ...
            temp = self.select_folder(self.json_widgets['pb_Specpath']['select_folder_text_browser'], self.json_widgets['pb_Specpath']['select_folder_win_title'])
            if temp:
    ...
    def contents_directory(self):
        try:
            if not self.Win.rb_OutputMethod_D.isChecked():
                return
            action_reply = self.set_custom_message_box(self.json_general['optional_option'], f'{self.json_special["contents_directory"]["msg_content"]}<br>', [self.json_general['pb_set']], True)
            # 重置键
            if action_reply == 0:
                self.cmd_dict['contents_directory'][0] = None
            ...
            temp = QInputDialog.getText(None, self.json_special['contents_directory']['dialog_title'], f'{self.json_special["contents_directory"]["dialog_content"]}<br>')[0]
            if temp:
    ...
    def add_file_folder_data(self):
        try:
            ...
            # 调用对话框
            temp_command_list= self.set_custom_list_widget(self.json_widgets['pb_AddFileFolderData']['text_browser_display'],self.json_widgets['pb_AddFileFolderData']['dialog_title'], content_list)
    ...
    def add_binary_data(self):
        try:
            ...
            # 调用对话框
            temp_command_list= self.set_custom_list_widget(self.json_widgets['pb_AddBinaryData']['text_browser_display'], self.json_widgets['pb_AddBinaryData']['dialog_title'], content_list, 'file_folder')
            # 数据处理，将对话框的输入内容转换为命令的格式
            if temp_command_list:
                command_list = []
                for i in temp_command_list:
    ...
    def imports_folder(self):
        try:
            ...
            # 调用对话框
            temp_command_list= self.set_custom_list_widget(self.json_widgets['pb_ImportsFolder']['text_browser_display'], self.json_widgets['pb_ImportsFolder']['dialog_title'], content_list, 'folder')
            # 数据处理，将对话框的输入内容转换为命令的格式
            if temp_command_list:
                command_list = []
                for i in temp_command_list:
    ...
    def import_module_name(self):
        try:
            ...
            # 调用对话框
            temp_command_list= self.set_custom_list_widget(self.json_widgets['pb_ImportModuleName']['text_browser_display'], self.json_widgets['pb_ImportModuleName']['dialog_title'], content_list, 'text', text_discription=f'{self.json_widgets["pb_ImportModuleName"]["sub_dialog_title"]}<br>{self.json_widgets["pb_ImportModuleName"]["sub_dialog_content"]}<br>')
            # 数据处理，将对话框的输入内容转换为命令的格式
            if temp_command_list:
                command_list = []
                for i in temp_command_list:
    ...
    def collect_submodules(self):
        try:
            ...
            # 调用对话框
            temp_command_list= self.set_custom_list_widget(self.json_widgets['pb_CollectSubmodules']['text_browser_display'], self.json_widgets['pb_CollectSubmodules']['dialog_title'], content_list, 'text', text_discription=f'{self.json_widgets["pb_CollectSubmodules"]["sub_dialog_title"]}<br>{self.json_widgets["pb_CollectSubmodules"]["sub_dialog_content"]}br>')
            # 数据处理，将对话框的输入内容转换为命令的格式
            if temp_command_list:
                command_list = []
                for i in temp_command_list:
    ...
    def collect_data(self):
        try:
            ...
            # 调用对话框
            temp_command_list= self.set_custom_list_widget(self.json_widgets['pb_CollectData']['text_browser_display'], self.json_widgets['pb_CollectData']['dialog_title'], content_list, 'text', text_discription=f'{self.json_widgets["pb_CollectData"]["sub_dialog_title"]}<br>{self.json_widgets["pb_CollectData"]["sub_dialog_content"]}<br>')
            # 数据处理，将对话框的输入内容转换为命令的格式
            if temp_command_list:
                command_list = []
                for i in temp_command_list:
    ...
    def collect_binaries(self):
        try:
            ...
            # 调用对话框
            temp_command_list= self.set_custom_list_widget(self.json_widgets['pb_CollectBinaries']['text_browser_display'], self.json_widgets['pb_CollectBinaries']['dialog_title'], content_list, 'text', text_discription=f'{self.json_widgets["pb_CollectBinaries"]["sub_dialog_title"]}<br>{self.json_widgets["pb_CollectBinaries"]["sub_dialog_content"]}<br>')
            # 数据处理，将对话框的输入内容转换为命令的格式
            if temp_command_list:
                command_list = []
                for i in temp_command_list:
    ...
    def collect_all(self):
        try:
            ...
            # 调用对话框
            temp_command_list= self.set_custom_list_widget(self.json_widgets['pb_CollectAll']['text_browser_display'], self.json_widgets['pb_CollectAll']['dialog_title'], content_list, 'text', text_discription=f'{self.json_widgets["pb_CollectAll"]["sub_dialog_title"]}<br>{self.json_widgets["pb_CollectAll"]["sub_dialog_content"]}<br>')
            # 数据处理，将对话框的输入内容转换为命令的格式
            if temp_command_list:
                command_list = []
                for i in temp_command_list:
    ...
    def copy_metadata(self):
        try:
            ...
            # 调用对话框
            temp_command_list= self.set_custom_list_widget(self.json_widgets['pb_CopyMetadata']['text_browser_display'], self.json_widgets['pb_CopyMetadata']['dialog_title'], content_list, 'text', text_discription=f'{self.json_widgets["pb_CopyMetadata"]["sub_dialog_title"]}<br>{self.json_widgets["pb_CopyMetadata"]["sub_dialog_content"]}<br>')
            # 数据处理，将对话框的输入内容转换为命令的格式
            if temp_command_list:
                command_list = []
                for i in temp_command_list:
    ...
    def recursive_copy_metadata(self):
        try:
            ...
            # 调用对话框
            temp_command_list= self.set_custom_list_widget(self.json_widgets['pb_RecursiveCopyMetadata']['text_browser_display'], self.json_widgets['pb_RecursiveCopyMetadata']['dialog_title'], content_list, 'text', text_discription=f'{self.json_widgets["pb_RecursiveCopyMetadata"]["sub_dialog_title"]}<br>{self.json_widgets["pb_RecursiveCopyMetadata"]["sub_dialog_content"]}<br>')
            # 数据处理，将对话框的输入内容转换为命令的格式
            if temp_command_list:
                command_list = []
                for i in temp_command_list:
    ...
    def additional_hooks_dir(self):
        try:
            ...
            # 调用对话框
            temp_command_list= self.set_custom_list_widget(self.json_widgets['pb_AdditionalHooksDir']['text_browser_display'], self.json_widgets['pb_AdditionalHooksDir']['dialog_title'], content_list, 'folder')
            # 数据处理，将对话框的输入内容转换为命令的格式
            if temp_command_list:
                command_list = []
                for i in temp_command_list:
    ...
    def runtime_hook(self):
        try:
            ...
            # 调用对话框
            temp_command_list= self.set_custom_list_widget(self.json_widgets['pb_RuntimeHook']['text_browser_display'], self.json_widgets['pb_RuntimeHook']['dialog_title'], content_list, 'file')
            # 数据处理，将对话框的输入内容转换为命令的格式
            if temp_command_list:
                command_list = []
                for i in temp_command_list:
    ...
    def exclude_module(self):
        try:
            ...
            # 调用对话框
            temp_command_list= self.set_custom_list_widget(self.json_widgets['pb_ExcludeModule']['text_browser_display'], self.json_widgets['pb_ExcludeModule']['dialog_title'], content_list, 'text', text_discription=f'{self.json_widgets["pb_ExcludeModule"]["sub_dialog_title"]}<br>{self.json_widgets["pb_ExcludeModule"]["sub_dialog_content"]}<br>')
            # 数据处理，将对话框的输入内容转换为命令的格式
            if temp_command_list:
                command_list = []
                for i in temp_command_list:
    ...
    def debug_mode(self):
        try:
            ...
            action_reply = self.set_custom_message_box(self.json_widgets['pb_DebugMode']['text_browser_display'], f'{self.json_widgets["pb_DebugMode"]["current_level"]}<br>{current_data}<br><br>{self.json_widgets["pb_DebugMode"]["dialog_title"]}<br>',[f'all{self.json_widgets["pb_DebugMode"]["all_translate"]}',f'imports{self.json_widgets["pb_DebugMode"]["imports_translate"]}',f'bootloader{self.json_widgets["pb_DebugMode"]["bootloader_translate"]}',f'noarchive{self.json_widgets["pb_DebugMode"]["noarchive_translate"]}'], True)
            if action_reply == 0:
    ...
    def noupx_option(self):
        try:
            if self.Win.cb_NoupxOption.isChecked():
                QMessageBox.information(None, self.json_general["msg_info"], f'{self.json_widgets["cb_NoupxOption"]["msg_content"]}<br>')
    ...
    def upx_exclude(self):
        try:
            ...
            temp = self.select_file(self.json_widgets['pb_UpxExclude']['text_browser_display'], self.json_widgets['pb_UpxExclude']['dialog_title'], f'{self.json_general["select_file"]}(*.*)')
            if temp:
    ...
    def hide_console(self): 
        try:
            ...
            action_reply = self.set_custom_message_box(self.json_widgets["pb_HideConsole"]["text_browser_display"], f'{self.json_widgets["pb_HideConsole"]["current_level"]}<br>{current_data}<br><br>{self.json_widgets["pb_HideConsole"]["dialog_title"]}<br>{self.json_widgets["pb_HideConsole"]["dialog_content"]}<br>',[f'minimize-late{self.json_widgets["pb_HideConsole"]["minimize_late_translate"]}',f'hide-early{self.json_widgets["pb_HideConsole"]["hide_early_translate"]}',f'minimize-early{self.json_widgets["pb_HideConsole"]["minimize_early_translate"]}',f'hide-late{self.json_widgets["pb_HideConsole"]["hide_late_translate"]}'], True)
            if action_reply == 0:
    ...
    def version_file(self):
        try:
            if self.cmd_dict['version_file'][0]:
                ...
                action_reply = self.set_custom_message_box(self.json_general['data_exist'], f'{self.json_general["current_data"]}{current_data}<br>{self.json_general["msg_file_replace"]}', [self.json_general['pb_replace']],True)
                if action_reply == 0:
            ...
            temp = self.select_file(self.json_widgets["pb_VersionFile"]["text_browser_display"], self.json_widgets["pb_VersionFile"]["dialog_title"], f'{self.json_general["text_file"]}(*.txt *.ini)')
            if temp:
    ...
    def add_xml_file(self):
        try:
            if self.cmd_dict['add_xml_file'][0]:
                ...
                action_reply = self.set_custom_message_box(self.json_general['data_exist'], f'{self.json_general["current_data"]}{current_data}<br>{self.json_general["msg_item_replace"]}<br>', [self.json_general['pb_replace']], reset_flag=True)
                # 重置键
                if action_reply == 0:
            ...
            temp = self.select_file(self.json_widgets['pb_AddXmlFile']['text_browser_display'], self.json_widgets['pb_AddXmlFile']['dialog_title'], f'{self.json_general["text_file"]}(*.*)')
            if temp:
    ...
    def add_resource(self):
        try:
            ...
            # 调用对话框
            temp_command_list= self.set_custom_list_widget(self.json_widgets['pb_AddResource']['text_browser_display'], self.json_widgets['pb_AddResource']['dialog_title'], content_list, extra_func='edit_info')
            # 数据处理，将对话框的输入内容转换为命令的格式
            if temp_command_list:
                command_list = []
                for i in temp_command_list:
    ...
    def osx_bundle_identifier(self):
        try:
            if self.cmd_dict['osx_bundle_identifier'][0]:
                ...
                action_reply = self.set_custom_message_box(self.json_general['data_exist'], f'{self.json_general["current_data"]}{current_data}<br>{self.json_general["msg_if_replace"]}<br>', [{self.json_general["pb_change"]}],True)
                if action_reply == 0:
            ...
            temp = QInputDialog.getText(None, self.json_widgets['pb_OsxBundleIdentifier']['dialog_title'], f'{self.json_widgets["pb_OsxBundleIdentifier"]["dialog_content"]}<br><br>')[0]
            if temp: 
    ...
    def target_architecture(self):
        try:
            if self.cmd_dict['target_architecture'][0]:
                ...
                action_reply = self.set_custom_message_box(self.json_general['data_exist'], f'{self.json_general["current_data"]}{current_data}<br>{self.json_general["msg_if_replace"]}<br>', ['x86', 'x86_64'],reset_flag=True)
                if action_reply == 0:
            ...
            action_reply = self.set_custom_message_box(self.json_widgets['pb_TargetArchitecture']['text_browser_display'], f'{self.json_widgets["pb_TargetArchitecture"]["dialog_title"]}<br>', ['x86', 'x86_64'])
            if action_reply == 1: 
    ...
    def codesign_identity(self):
        try:
            ...
            temp = QInputDialog.getText(None, self.json_widgets['pb_CodesignIdentity']['text_browser_display'], f'{self.json_widgets["pb_CodesignIdentity"]["dialog_title"]}<br>')[0]
            if temp: 
    ...
    def osx_entitlements_file(self):
        try:
            ...
            temp = self.select_file(self.json_widgets['pb_OsxEntitlementsFile']['text_browser_display'], self.json_widgets['pb_OsxEntitlementsFile']['dialog_title'], f'entitlements {self.json_general["file"]}(*.plist')
            if temp:
    ...
    def runtime_tmpdir(self):
        try:
            if self.cmd_dict['output_methode'][0] == 'pyinstaller -D':
                ...
                QMessageBox.information(None, self.json_general["msg_info"], self.json_widgets['pb_RuntimeTmpdir']['msg_info'])
                return
            if self.cmd_dict['runtime_tmpdir'][0]:
                current_data = self.cmd_dict['runtime_tmpdir'][0]
                action_reply = self.set_custom_message_box(self.json_general['data_exist'], f'{self.json_general["current_data"]}{current_data}<br>{self.json_general["msg_file_replace"]}<br>', [self.json_general['pb_replace']])
                if action_reply == 0:
            ...
            temp = self.select_folder(self.json_widgets['pb_RuntimeTmpdir']['text_browser_display'], self.json_widgets['pb_RuntimeTmpdir']['dialog_title'])
            if temp:
    ...
    def workpath_option(self):
        try:
            ...
            temp = self.select_folder(self.json_widgets['pb_WorkpathOption']['text_browser_display'], self.json_widgets['pb_WorkpathOption']['dialog_title'])
            if temp:
    ...
    def upx_dir(self):
        try:
            ...
            temp = self.select_folder(self.json_widgets['pb_UpxDir']['text_browser_display'], self.json_widgets['pb_UpxDir']['dialog_title'])
            if temp:
    ...
    def launch_cmd(self):
        ...
        if not self.launch_flag or not os.path.exists(self.Win.pte_FilePath.toPlainText()):
            QMessageBox.information(None, self.json_general["msg_info"], self.json_special['launch_cmd']['msg_content_no_file'])
            return
        if not os.path.exists(self.Win.pte_OutputPath.toPlainText()):
            QMessageBox.information(None, self.json_general["msg_info"], self.json_special['launch_cmd']['msg_content_no_folder'])
            return
    ...
    def thread_finished_file_del(self):
        ...
        try:
            if self.clear_file_flag:
                ...
                self.append_TB_text(f'__________ {self.json_general["deleted_file"]} __________\n', self.Win.textBrowser)
        ...
        try:
            ...
            for path_item in enviro_path_list:
                if 'Python' in os.path.basename(os.path.dirname(path_item)):
                    ...
                    break
                else: python_path = None
            if not python_path: 
                QMessageBox.information(None, self.json_general["msg_info"], self.json_special['pip_upgrade']['no_python'])
                return
            self.launch_thread = Pip_Upgrade_Thread(python_path)
            self.launch_thread.output_to_textbrowser_cmd.connect(lambda content: self.append_TB_text(content, self.Win.textBrowser_cmd))
