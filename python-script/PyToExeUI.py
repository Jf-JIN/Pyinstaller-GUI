...
class PyToExeUI(Ui_MainWindow):
    ...
    def py_to_exe_ui_signal_connection(self):
        ...
        self.Win.cbb_LanguageSelect.currentTextChanged.connect(
...
# 界面文字初始化及更改
        ...
        try:
            ...
            for file in file_list:
                if file.endswith('.json'):
                    ...
                    self.Win.cbb_LanguageSelect.addItem(
    ...
    def set_text_init(self):
        try:
            ...
            self.setWindowTitle(
                self.json_special['window_title'] + '\t\t\t\t' + workspace_path)
        except Exception as e:
            # traceback.print_exc()
            self.append_TB_text(
                f'__________ {self.json_general["error"]} __________\n{e}\n', self.Win.textBrowser_cmd)
    ...
    def clear_input(self):
        action_reply = self.set_custom_message_box(self.json_general["msg_warning"], self.json_widgets['pb_ClearInput']['msg_content'], [
                                                   self.json_widgets['pb_ClearInput']['pb_certain_clear']], messagebox_content_icon=QMessageBox.Warning)
        if action_reply == 1:
            pass
        else:
            return False
        ...
        for value in self.cmd_dict.values():
            value[0] = None
            value[2] = None
        self.recover_cb = [[checkbox_item, checkbox_item.isChecked(
        )] for checkbox_item in self.Win.frame_cb.findChildren(QCheckBox)]
        self.recover_rb = [[radio_button_item, radio_button_item.isChecked(
        )] for radio_button_item in self.Win.frame_rb.findChildren(QRadioButton)]
        self.recover_lock = [[radio_button_item, radio_button_item.isChecked(
        )] for radio_button_item in self.Win.frame_FileInfo.findChildren(QRadioButton)]
        ...
        # 重置参数
        reset_py_file_path = self.recover_cmd_dict['python_file_path'][0].split('"')[
            1]
        self.Win.pte_FilePath.setPlainText(reset_py_file_path)
        self.Win.pte_OutputPath.setPlainText(
            os.path.dirname(reset_py_file_path))
        self.Win.pte_FileName.setPlainText(
            os.path.splitext(os.path.basename(reset_py_file_path))[0])
        self.cmd_dict['output_methode'][0] = 'pyinstaller -F'
        self.cmd_dict['output_methode'][2] = self.json_widgets['rb_OutputMethod_F']['dict_explain']
        self.cmd_dict['clear_cache'][0] = '--clean'
        self.cmd_dict['clear_cache'][2] = self.json_widgets['cb_ClearCache']['dict_explain']
    ...
    def recover(self):
        try:
            ...
            if self.recover_cmd_dict['python_file_path'][0]:
                self.Win.pte_FilePath.setPlainText(
                    self.recover_cmd_dict['python_file_path'][0].split('"')[1])
            if self.recover_cmd_dict['output_folder_path'][0]:
                self.Win.pte_OutputPath.setPlainText(
                    self.recover_cmd_dict['output_folder_path'][0].split('"')[1])
            if self.recover_cmd_dict['output_file_name'][0]:
                self.Win.pte_FileName.setPlainText(
                    self.recover_cmd_dict['output_file_name'][0].split('"')[1])
            self.Win.pb_Recover.hide()
        except Exception as e:
            # traceback.print_exc()
            self.append_TB_text(
    ...
    def parameter_display(self):
        try:
            self.append_TB_text(
                f'__________  {self.json_general["display_command_parameter"]}  __________\n')
            for value in self.cmd_dict.values():
                if value[0]:
                    command_display = str(
                        self.json_widgets[value[1]]["dict"] + value[2])
                    self.append_TB_text(command_display, self.Win.textBrowser)
            self.append_TB_text(
                f'__________  {self.json_general["all_parameter"]}  __________\n\n')
        except Exception as e:
            # traceback.print_exc()
    ...
    # ****************************************打印参数到txt****************************************
    def print_cmd(self):
        print_command_path = os.path.join(
            workspace_path, 'output_command_of_pyinstaller.txt')
        ...
        try:
            ...
            QMessageBox.information(
                None, self.json_general["msg_info"], f'{self.json_special["print_cmd"]}<br>{print_command_path}')
    ...
    def open_output_folder(self):
        try:
            if self.cmd_dict['python_file_path'][0] and os.path.exists(self.cmd_dict['output_folder_path'][0].split('"')[1]):
                folder_path = self.cmd_dict['output_folder_path'][0].split('"')[
                    1]
            elif workspace_path:
                QMessageBox.information(
                    None, self.json_general["msg_info"], self.json_special['open_output_folder']['no_folder'])
                folder_path = workspace_path
            else:
                QMessageBox.information(
                    None, self.json_general["msg_info"], self.json_special['open_output_folder']['no_file'])
                return
            QDesktopServices.openUrl(
    ...
    def select_py_file(self):
        temp = self.select_file(self.json_special['select_py_file']['text_browser_display'],
                                self.json_special['select_py_file']['dialog_title'], self.json_special['select_py_file']['type_discription'])
        self.Win.pte_FilePath.setPlainText(temp)
        self.cmd_dict['python_file_path'][0].split('"')[1] = temp
        self.cmd_dict['python_file_path'][0] = ''.join(
            self.cmd_dict['python_file_path'][0])

    def select_ourput_folder(self):
        temp = self.select_folder(
            self.json_special['select_ourput_folder']['text_browser_display'], self.json_special['select_ourput_folder']['dialog_title'])
        self.Win.pte_OutputPath.setPlainText(temp)
        self.cmd_dict['output_folder_path'][0].split('"')[1] = temp
        self.cmd_dict['output_folder_path'][0] = ''.join(
            self.cmd_dict['output_folder_path'][0])
    ...
    def select_file(self, display_text: str = '', window_title: str = '', file_discription: str = '') -> str:
        ...
        try:
            receiver_temp, _ = QFileDialog.getOpenFileName(
                self, f'{window_title}', '', f'{file_discription}', options=options)
            if receiver_temp:
                self.append_TB_text(
                    f'__________ {self.json_general["setting_update"]}{display_text} __________\n{receiver_temp}\n')
    ...
    def select_folder(self, display_text: str = '', window_title: str = '') -> str:
        ...
        try:
            receiver_temp = QFileDialog.getExistingDirectory(
                self, f'{window_title}', '', options=options)
            if receiver_temp:
                self.append_TB_text(
                    f'__________ {self.json_general["setting_update"]}{display_text} __________\n{receiver_temp}\n')
                return receiver_temp
    ...
    def type_select_dialog(self, display_text, file_type=None):
        ...
        action_reply = self.set_custom_message_box(self.json_special['type_select_dialog']['msg_title'], self.json_special['type_select_dialog']['msg_content'], [
                                                   self.json_general['folder'], self.json_general['file']])
        if action_reply == 1:
            temp = self.select_folder(display_text, )
        elif action_reply == 2:
            temp = self.select_file(
                display_text, self.json_general['select_file'], file_type)
        else:
            return
        return temp
    ...
    def set_custom_list_widget(self, display_text, window_title, item_list, type_select_flag='file_folder', file_type=None, text_discription=None, extra_func=None, window_icon=None):
        ...
        def preview_image():
            ...
            if current_row >= 0 and current_row <= list_widget.count():
                ...
                lb_preview.setPixmap(pixmap.scaled(
                    lb_preview.size(), Qt.KeepAspectRatio))

        def edit_info():
            current_items = [list_widget.item(
                i).text() for i in range(list_widget.count())]
            current_row = list_widget.currentRow()
            if current_row >= 0 and current_row < list_widget.count():
                original = list_widget.item(current_row).text()
                temp_input, _ = QInputDialog.getText(
                    None, self.json_general['input_specified_data'], self.json_widgets['pb_AddResource']['msg_content'])
                if temp_input:
                    temp_content = original.split(',')[0] + ',' + temp_input
                if temp_content in current_items:
                    QMessageBox.information(
                        None, self.json_general["msg_info"], f'{self.json_general["redundant_info"]}<br>{temp_input}<br>')
                else:
                    list_widget.item(current_row).setText(temp_content)

        def line_edit_input():
            temp = QInputDialog.getText(
                None, self.json_general['input_specified_data'], text_discription)[0]
            if temp:
                return temp
            else:
                return None
        ...
        def add_item():
            ...
            if type_select_flag == 'file' or type_select_flag == 'image':
                ...
                temp = self.select_folder(
                    display_text, self.json_general['select_folder'])
            ...
            if temp in current_items:
                QMessageBox.information(
                    None, self.json_general["msg_info"], f'{self.json_general["redundant_info"]}<br>{temp}<br>')
            else:
        ...
        if result == QDialog.Accepted:
            selected_items = [list_widget.item(
                i).text() for i in range(list_widget.count())]
